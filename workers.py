...
def clean_version(version_str):
    """Hilfsfunktion: Filtert Buchstaben heraus, lässt nur die Versionsnummer übrig."""
    if not version_str:
        return ""
...
# ==========================================
# NEU: Der Scan-Arbeiter (Sucht im Hintergrund nach Updates)
# ==========================================
class ScanWorker(QThread):
    progress_update = pyqtSignal(int, int) # Sendet (Aktueller_Stand, Gesamt_Anzahl)
    status_update = pyqtSignal(str)
    addon_scanned = pyqtSignal(str, str, str, str) # Sendet (Name, Lokal_V, Online_V, Update_URL)
    finished = pyqtSignal()
    ...
    def run(self):
        ...
        for i, (name, data) in enumerate(addons.items()):
            ...
            # Wir speichern uns die curse_id in einer lokalen Variable, 
            # damit wir sie überschreiben können, falls wir sie scrapen
            current_curse_id = data["curse_id"]
            
            # ==========================================
            # NEU: Das Web-Scraping Fallback
            # ==========================================
            # Wenn wir absolut KEINE Quelle haben, versuchen wir unser Glück beim Scraper
            ...
            # WICHTIG: Hier jetzt current_curse_id statt data["curse_id"] verwenden!
            ...
            # Signal an die GUI senden, DIESES eine Addon sofort in die Tabelle zu zeichnen
            self.addon_scanned.emit(name, clean_local, clean_online or "-", actual_update_url)
        
        self.progress_update.emit(total, total) # Auf 100% setzen
        self.status_update.emit("Scan abgeschlossen!")
        self.finished.emit()

# ==========================================
# Der Update-Arbeiter (Lädt herunter)
# ==========================================
class UpdateWorker(QThread):
    progress_update = pyqtSignal(int, int) # NEU: Fortschritt für Downloads
    status_update = pyqtSignal(str) 
    finished = pyqtSignal()         
